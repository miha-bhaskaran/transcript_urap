...
def extract_tables_from_response(response):
    ...
    #print(f"Found {len(tables)} tables.")  # Debug statement
                        ...
                        if relationship['Type'] == 'CHILD':
                            for child_id in relationship['Ids']:
                                ...
                                if word['BlockType'] == 'WORD':
                                    text += word['Text'] + ' '
                    text = text.strip()
                    # Debug statement to check the content of each cell
                    #print(f"Row {row_index}, Column {col_index}: {text}")
                else:
                    text = ''
...
def process_images_in_folder(folder_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith('.png'):
            ...
            # Debug: Print the extracted tables
            # for i, table in enumerate(tables):
            #     print(f"Table {i+1} in {filename}:")
            #     for row in table:
            #         print(row)
...
#folder_path = 'fake'  # Replace with the path to your folder containing PNG files
