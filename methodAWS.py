...
def draw_bounding_boxes_on_image(image, blocks):
    ...
    for block in blocks:
        ...
        if block['BlockType'] == 'CELL' and 'Geometry' in block:
            ...
            shading_color = (255, 0, 0, intensity)  # Red color with variable transparency

            # Draw shaded rectangle
            draw.rectangle([left, top, left + box_width, top + box_height], fill=shading_color)
...
def process_images_in_folder(folder_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith('.png'):
            ...
            image_with_boxes.save(os.path.join(folder_path, f"{os.path.splitext(filename)[0]}_bounding.png"), "PNG")

            tables = extract_tables_from_response(response)

            # Save each table to its own CSV file
            for i, table in enumerate(tables):
                csv_filename = os.path.join(folder_path, f"{os.path.splitext(filename)[0]}_table_{i + 1}.csv")
                save_table_to_csv(table, csv_filename)
                print(f"Extracted table {i + 1} from {filename} has been saved to {csv_filename}")
